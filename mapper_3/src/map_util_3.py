    ...
    def UpdateMapDisplay(self, new_map, position, real_position):
        for ii in np.ndindex(self.map_size):
            if self.map[ii] == int(new_map[ii]):
                continue
            if real_position[1] < 105 and real_position[0] < 16:
            ...
            elif real_position[1] >= 105 and real_position[0] < 70:
                self.map[ii] = int(new_map[ii])
                pt1 = (int(self.draw_scale*ii[1]), int(self.draw_scale*ii[0]))
                pt2 = (int(self.draw_scale*(ii[1]+ 1)), int(self.draw_scale*(ii[0] + 1)))
                color = self.map_colors[self.map[ii] + 2]  
                cv2.rectangle(self.drawn_map, pt1, pt2, color, -1) #pt1 pt2 left upper corner and right corner -1 fill the rectangle
            elif real_position[1] < 15 and 16 <= real_position[0] < 70:
                self.map[ii] = int(new_map[ii])
                pt1 = (int(self.draw_scale*ii[1]), int(self.draw_scale*ii[0]))
                pt2 = (int(self.draw_scale*(ii[1]+ 1)), int(self.draw_scale*(ii[0] + 1)))
                color = self.map_colors[self.map[ii] + 5]  
                cv2.rectangle(self.drawn_map, pt1, pt2, color, -1) #pt1 pt2 left upper corner and right corner -1 fill the rectangle
            elif 15 <= real_position[1] < 25 and 42 <= real_position[0] < 60:
                self.map[ii] = int(new_map[ii])
                pt1 = (int(self.draw_scale*ii[1]), int(self.draw_scale*ii[0]))
                pt2 = (int(self.draw_scale*(ii[1]+ 1)), int(self.draw_scale*(ii[0] + 1)))
                color = self.map_colors[self.map[ii] + 5]  
            ...
            elif 15 <= real_position[1] < 95 and 60 <= real_position[0] < 70:
                self.map[ii] = int(new_map[ii])
                pt1 = (int(self.draw_scale*ii[1]), int(self.draw_scale*ii[0]))
                pt2 = (int(self.draw_scale*(ii[1]+ 1)), int(self.draw_scale*(ii[0] + 1)))
                color = self.map_colors[self.map[ii] + 5]  
            ...
            elif 95 <= real_position[1] < 105 and 16 <= real_position[0] < 70:
                self.map[ii] = int(new_map[ii])
                pt1 = (int(self.draw_scale*ii[1]), int(self.draw_scale*ii[0]))
                pt2 = (int(self.draw_scale*(ii[1]+ 1)), int(self.draw_scale*(ii[0] + 1)))
                color = self.map_colors[self.map[ii] + 5]  
        ...
        cv2.waitKey(5)
        cv2.imwrite('/home/hanke/catkin_ws/src/mapper_3/ekf_1imu1gps_friction.jpg', img)
